defmodule GodwokenExplorer.ExchangeRatesTest do
  use ExUnit.Case, async: false

  import Mox

  alias Plug.Conn
  alias GodwokenExplorer.ExchangeRates
  alias GodwokenExplorer.ExchangeRates.Token
  alias GodwokenExplorer.ExchangeRates.Source.TestSource
  ...
  setup do
    ...
    source_configuration =
      Application.get_env(:godwoken_explorer, GodwokenExplorer.ExchangeRates.Source)

    rates_configuration = Application.get_env(:godwoken_explorer, GodwokenExplorer.ExchangeRates)

    Application.put_env(:godwoken_explorer, GodwokenExplorer.ExchangeRates.Source,
      source: TestSource
    )

    Application.put_env(:godwoken_explorer, GodwokenExplorer.ExchangeRates, table_name: :rates)
    Application.put_env(:godwoken_explorer, GodwokenExplorer.ExchangeRates, enabled: true)

    on_exit(fn ->
      Application.put_env(
        :godwoken_explorer,
        GodwokenExplorer.ExchangeRates.Source,
        source_configuration
      )

      Application.put_env(:godwoken_explorer, GodwokenExplorer.ExchangeRates, rates_configuration)
    end)
  end
  ...
  test "lookup when disabled" do
    Application.put_env(:godwoken_explorer, GodwokenExplorer.ExchangeRates, enabled: false)
