defmodule GodwokenRPC.Transaction.FetchedTransactions do
  alias GodwokenRPC.Transaction.FetchedTransaction
  ...
  def from_responses(responses, id_to_params) do
    responses
    |> Enum.map(&FetchedTransaction.from_response(&1, id_to_params))
    |> Enum.reduce(
      %__MODULE__{},
      fn
  ...
  def requests(id_to_params) when is_map(id_to_params) do
    Enum.map(id_to_params, fn {id, %{gw_tx_hash: tx_hash}} ->
      FetchedTransaction.request(%{id: id, gw_tx_hash: tx_hash})
    end)
  end
end
