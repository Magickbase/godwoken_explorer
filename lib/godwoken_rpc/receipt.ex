defmodule GodwokenRPC.Receipt do
  @moduledoc """
  ...
  @typedoc """
   * `"contractAddress"` - The contract `t:GodwokenRPC.address/0` created, if the transaction was a contract
     creation, otherwise `nil`.
   * `"blockHash"` - `t:GodwokenRPC.hash/0` of the block where `"transactionHash"` was in.
  ...
  @doc """
  Get `t:GodwokenRPC.Logs.elixir/0` from `t:elixir/0`
  """
  ...
  Converts `t:elixir/0` format to params used in `Explorer.Chain`.
      ...
      iex> GodwokenRPC.Receipt.elixir_to_params(
      ...>   %{
  ...
  defp elixir_to_status(%{"status" => :ok}), do: :succeed
  defp elixir_to_status(_), do: :failed
