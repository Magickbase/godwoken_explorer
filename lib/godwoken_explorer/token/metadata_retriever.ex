defmodule GodwokenExplorer.Token.MetadataRetriever do
  @moduledoc """
  ...
  * Given that some of them were read:
  ...
  def set_skip_metadata(token_to_update) do
    Chain.update_udt(
      %{token_to_update | updated_at: NaiveDateTime.utc_now() |> NaiveDateTime.truncate(:second)},
      %{skip_metadata: true}
    )
  end
  ...
  defp fetch_functions_from_contract(contract_address_hash, contract_functions) do
    max_retries = Application.get_env(:explorer, :token_functions_reader_max_retries) || 2

    fetch_functions_with_retries(contract_address_hash, contract_functions, %{}, max_retries)
  end

  defp fetch_functions_with_retries(_contract_address_hash, _contract_functions, accumulator, 0),
    do: accumulator

  defp fetch_functions_with_retries(
         contract_address_hash,
         contract_functions,
         accumulator,
         retries_left
    ...
    contract_functions_result =
      Reader.query_contract(contract_address_hash, @contract_abi, contract_functions, false)
  ...
  defp handle_invalid_strings(
         %{name: name, symbol: symbol} = contract_functions,
         contract_address_hash
       ) do
