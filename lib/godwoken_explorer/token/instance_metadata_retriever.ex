defmodule GodwokenExplorer.Token.InstanceMetadataRetriever do
  @moduledoc """
  ...
  def fetch_metadata(contract_address_hash, token_id) do
    ...
    if res == {:ok, %{error: @vm_execution_error}} do
      hex_normalized_token_id =
        token_id |> Integer.to_string(16) |> String.downcase() |> String.pad_leading(64, "0")
  ...
  def fetch_json(uri, _hex_token_id)
      when uri in [%{@token_uri => {:ok, [""]}}, %{@uri => {:ok, [""]}}] do
  ...
  defp fetch_json_from_uri({:ok, ["Qm" <> _ = result]}, hex_token_id) do
    if String.length(result) == 46 do
      ...
      Logger.debug(["Unknown metadata format result #{inspect(result)}."],
  ...
  defp fetch_json_from_uri({:ok, ["data:application/json," <> json]}, hex_token_id) do
    ...
    e ->
      Logger.debug(
        [
          "Unknown metadata format #{inspect(json)}.",
  ...
  defp fetch_metadata_inner(uri, hex_token_id) do
    prepared_uri = substitute_token_id_to_token_uri(uri, hex_token_id) |> IO.inspect()
    fetch_metadata_from_uri(prepared_uri, hex_token_id)
  rescue
    e ->
      Logger.debug(
        [
          "Could not prepare token uri #{inspect(uri)}.",
          Exception.format(:error, e, __STACKTRACE__)
        ],
        fetcher: :token_instances
      )
  ...
  def fetch_metadata_from_uri_inner(uri, hex_token_id) do
    case HTTPoison.get(uri, [],
      ...
      {:ok, %Response{body: body, status_code: code}} ->
        Logger.debug(
          [
            "Request to token uri: #{inspect(uri)} failed with code #{code}. Body:",
            inspect(body)
          ],
          fetcher: :token_instances
        )
    ...
    e ->
      Logger.debug(
        [
          "Could not send request to token uri #{inspect(uri)}.",
          Exception.format(:error, e, __STACKTRACE__)
        ],
        fetcher: :token_instances
      )
  ...
  defp substitute_token_id_to_token_uri(token_uri, empty_token_id)
