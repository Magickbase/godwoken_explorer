...
defmodule GodwokenExplorer.SmartContract.Solidity.Verifier do
  @moduledoc """
  ...
  alias GodwokenExplorer.SmartContract.Solidity.CodeCompiler
  alias GodwokenExplorer.SmartContract.Verifier.ConstructorArguments
  ...
  defp verify(address_hash, params, json_input) do
    ...
    autodetect_constructor_arguments =
      params |> Map.get("autodetect_constructor_args", "false") |> parse_boolean()
  ...
  defp extract_settings_from_json(json_input) when is_map(json_input) do
    %{"enabled" => optimization, "runs" => optimization_runs} =
      json_input["settings"]["optimizer"]

    %{"optimization" => optimization}
    |> (&if(parse_boolean(optimization),
          do: Map.put(&1, "optimization_runs", optimization_runs),
          else: &1
        )).()
  end
  ...
  defp verify(address_hash, params) do
    ...
    autodetect_constructor_arguments =
      params |> Map.get("autodetect_constructor_args", "true") |> parse_boolean()
  ...
  defp compare_bytecodes(
    ...
    cond do
      ...
      has_constructor_with_params?(abi) && autodetect_constructor_arguments ->
        ...
        result_2 =
          ConstructorArguments.find_constructor_arguments(
            address_hash,
            abi,
            contract_source_code,
            contract_name
  ...
  defp try_to_verify_with_unknown_constructor_args(
         creation_code,
         generated_bytecode,
         trimmed_bytecode,
         abi
  ...
  defp do_extract_bytecode_and_metadata_hash(
         extracted,
         remaining,
         metadata_hash,
         compiler_version
    ...
    case remaining do
      ...
      @metadata_hash_prefix_0_4_23 <>
          <<metadata_hash::binary-size(64)>> <> "0029" <> _constructor_arguments ->
        do_extract_bytecode_and_metadata_hash_output(metadata_hash, extracted, compiler_version)
      ...
      <<_::binary-size(2)>> <>
          ...
          "43" <>
          <<compiler_version::binary-size(6)>> <> <<_::binary-size(4)>> <> _constructor_arguments ->
        do_extract_bytecode_and_metadata_hash_output(metadata_hash, extracted, compiler_version)
      ...
      <<_::binary-size(2)>> <>
          ...
          "78" <>
          <<_::binary-size(2)>> <>
          <<compiler_version::binary-size(76)>> <>
          "00" <> <<_::binary-size(2)>> <> _constructor_arguments ->
        do_extract_bytecode_and_metadata_hash_output(metadata_hash, extracted, compiler_version)
      ...
      <<_::binary-size(2)>> <>
          ...
          "78" <>
          <<_::binary-size(2)>> <>
          <<compiler_version::binary-size(78)>> <>
          "00" <> <<_::binary-size(2)>> <> _constructor_arguments ->
        do_extract_bytecode_and_metadata_hash_output(metadata_hash, extracted, compiler_version)
      ...
      <<_::binary-size(2)>> <>
          ...
          "78" <>
          <<_::binary-size(2)>> <>
          <<compiler_version::binary-size(80)>> <>
          "00" <> <<_::binary-size(2)>> <> _constructor_arguments ->
        do_extract_bytecode_and_metadata_hash_output(metadata_hash, extracted, compiler_version)
      ...
      <<_::binary-size(2)>> <>
          ...
          "78" <>
          <<_::binary-size(2)>> <>
          <<compiler_version::binary-size(82)>> <>
          "00" <> <<_::binary-size(2)>> <> _constructor_arguments ->
        do_extract_bytecode_and_metadata_hash_output(metadata_hash, extracted, compiler_version)
      ...
      <<next::binary-size(2)>> <> rest ->
        do_extract_bytecode_and_metadata_hash(
          [next | extracted],
          rest,
          metadata_hash,
          compiler_version
  ...
  def previous_evm_versions(current_evm_version) do
    index =
      Enum.find_index(CodeCompiler.allowed_evm_versions(), fn el -> el == current_evm_version end)
