defmodule GodwokenExplorer.SmartContract.Solidity.CodeCompiler do
  @moduledoc """
  Module responsible to compile the Solidity code of a given Smart Contract.
  """

  alias GodwokenExplorer.SmartContract.SolcDownloader
  ...
  ## Examples
      ...
      iex(1)> GodwokenExplorer.SmartContract.Solidity.CodeCompiler.run([
      ...>      name: "SimpleStorage",
    ...
    if path do
      {response, _status} =
        System.cmd(
          "node",
          [
            Application.app_dir(:godwoken_explorer, "priv/compile_solc.js"),
           ...
           {response, _status} <-
             System.cmd(
               ...
               [
                 Application.app_dir(
                   :godwoken_explorer,
                   "priv/compile_solc_standard_json_input.js"
                 ),
  ...
  defp fetch_candidates(contracts, "") when is_map(contracts) do
    candidates =
      for {file, content} <- contracts,
          {contract_name, %{"abi" => abi, "evm" => %{"bytecode" => %{"object" => bytecode}}}} <-
            content,
          do: %{
            "abi" => abi,
            "bytecode" => bytecode,
  ...
  defp get_contracts_standard_input_verification(%{"contracts" => contracts}),
    do: {:ok, contracts}
