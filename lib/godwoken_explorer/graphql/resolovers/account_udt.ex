defmodule GodwokenExplorer.Graphql.Resolvers.AccountUDT do
  ...
  defp search_account_current_brideged_udts(address_hashes, script_hashes, udt_script_hash) do
    ...
    query =
      ...
      |> where(
        [cbu, _a1, _a2, u],
        not is_nil(u.bridge_account_id) and not is_nil(cbu.value)
      )
    ...
    if length(address_hashes) > @addresses_max_limit or
      ...
      if account_query do
        ...
        cus =
          ...
          |> where([cu, u], not is_nil(cu.value) and not is_nil(u.name))
        ...
        cbus =
          ...
          |> where([cbu, u], not is_nil(cbu.value) and not is_nil(u.bridge_account_id))
