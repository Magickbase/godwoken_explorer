defmodule GodwokenExplorerWeb.RootController do
  ...
  def index(conn, _params) do
    api = %{
      ...
      account_show_url: "#{conn.scheme}://#{conn.host}/api/accounts/{account_id | short_script_hash | eth_address | layer1_script_hash}",
      search_url: "#{conn.scheme}://#{conn.host}/api/search?keyword={layer1_lock_script | block_hash | tx_hash | account_id | short_script_hash | eth_address | layer1_script_hash | account_id}"
    }
    json(conn, api)
  end
end
